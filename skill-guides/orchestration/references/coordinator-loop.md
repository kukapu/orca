...
For a fresh Claude, Codex, Cursor, OpenCode, or Pi terminal, `--model` accepts an opaque
...
Pi maps `--effort` to `--thinking`; OpenCode does not support launch-time `--effort`.
OpenCode model IDs may contain nested provider/model paths.
