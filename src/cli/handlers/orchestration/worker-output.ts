...
export function formatWorkerRead(
  ...
  // Keep the fork's partial-page hint inside the new details-first layout.
  const notes =
    value.source === 'transcript' && value.transcript.limited
      ? ['Transcript is partial; continue with the cursor.']
      : []
  if (output || notes.length > 0) {
    return [details, output, ...notes].filter(Boolean).join('\n\n')
  }
  const emptyMessage =
