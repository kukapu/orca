export function getStatusPluginMessagePreviewSource(): string[] {
  return [
    ...
    '  const options = messageOptionsById.get(pending.messageID);',
    '  await postMessagePart({',
    '    role: pending.role,',
    '    text: capMessagePartText(pending.text),',
    '    messageID: pending.messageID,',
    '    sessionID: pending.sessionID,',
    '    ...(options ? { model: options.model, ...(options.variant ? { variant: options.variant } : {}) } : {}),',
    '  }, pending.factoryID);',
    '}',
    '',
    ...
    '}',
    '',
    '// Why: only assistant messages carry execution evidence (providerID/modelID on',
    '// the reply). The user message model is a client selection, and a compaction',
    '// summary runs on a different model than the worker turn, so both are excluded.',
    'const messageOptionsById = new Map();',
    'function rememberMessageOptions(info) {',
    '  if (!info || info.role !== "assistant" || info.summary === true) return;',
    '  const messageID = info.id;',
    '  const providerID = typeof info.providerID === "string" ? info.providerID : "";',
    '  const modelID = typeof info.modelID === "string" ? info.modelID : "";',
    '  if (!messageID || !providerID || !modelID) return;',
    '  const variant = typeof info.variant === "string" && info.variant ? info.variant : null;',
    '  if (messageOptionsById.size >= 128) {',
    '    const first = messageOptionsById.keys().next().value;',
    '    if (first !== undefined) messageOptionsById.delete(first);',
    '  }',
    '  messageOptionsById.set(messageID, { model: `${providerID}/${modelID}`, variant });',
    '}',
    ''
  ]
}
