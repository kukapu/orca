...
export function resolveFederatedHomeRunId(
  attachment: {
    dispatch_id: string
    home_run_id?: string | null
  },
  db?: {
    getRunRaw?(runId: string): unknown
    db?: { pragma: (sql: string) => unknown }
  }
): string {
  if (attachment.home_run_id) {
    return attachment.home_run_id
  }
  const stub = federatedStubHomeRunId(attachment.dispatch_id)
  if (db?.getRunRaw?.(stub)) {
    return stub
  }
  const columns = db?.db?.pragma('table_info(remote_dispatch_attachments)') as
    | { name: string }[]
    | undefined
  if (columns && !columns.some((column) => column.name === 'home_run_id')) {
