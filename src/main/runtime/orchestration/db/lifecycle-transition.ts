...
const LEGAL_TRANSITIONS: Record<LifecycleEntity, Record<string, readonly string[]>> = {
  ...
  worker: {
    ...
    succeeded: ['succeeded'],
    // Why wider than terminal: a stalled-prompt `failed` worker can still be fenced
    // (stop, abandon) and a `failed`/process_exited row is adopted as `stopped` when
    // the exit handler already recorded the operator close this stop requested.
    failed: ['failed', 'stopping', 'stopped', 'abandoned'],
    stopped: ['stopped'],
    abandoned: ['abandoned']
  }
}
...
export function transitionLifecycleWithDb(
  ...
  // Why the recovery states: stop/abandon recovery may already have moved a worker
  // from `failed` into `stopping`/`stop_unknown` when its own late report lands — the
  // report is first-hand evidence and still supersedes whichever state it corrects.
  const promptReportCorrection =
    params.correction === 'unobserved_prompt_report' &&
    ['failed', 'stopping', 'stop_unknown'].includes(current.state) &&
    ((params.entity === 'task' && params.to === 'completed') ||
      (params.entity === 'dispatch' && params.to === 'completed') ||
      (params.entity === 'worker' && (params.to === 'succeeded' || params.to === 'failed')))
