...
export function createStartingWorkerDispatch(
  ...
  try {
    ...
    if (params.retryOf) {
      const retrySource = this.getDispatchContextById(params.retryOf)
      const retrySourceWorker = this.getWorkerDispatch(params.retryOf)
      // Why: a context-only Dispatch has no worker row, so its settled state lives on the Dispatch row.
      const retrySourceSettled = retrySourceWorker
        ? ['failed', 'stopped', 'abandoned'].includes(retrySourceWorker.state)
        : retrySource?.status === 'failed'
      if (
        !retrySource ||
        retrySource.task_id !== task.id ||
        prior?.id !== retrySource.id ||
        !retrySourceSettled ||
