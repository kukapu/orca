        ...
        if (federated) {
          ...
          try {
            ...
            if (remote.state === 'succeeded' || remote.state === 'failed') {
              // An unobserved-prompt failure settles from its own `failed` record via the
              // relay import; resuming would rewrite the worker as ready before its report lands.
