...
import type { WorkerStartInput, WorkerStartMutation } from './worker-start-schema'
import { runWorkerTerminalReadinessGate } from './worker-start-readiness'
...
export async function startLocalWorker(args: {
  ...
  // Structured sessions use host create-support, not PATH-based CLI detection.
  ...
  try {
    ...
    await runWorkerTerminalReadinessGate({
      runtime,
      setupStage,
      structuredSession,
      agent: agent as TuiAgent,
      reusedTerminal: params.terminal,
      timeoutMs: params.timeoutMs ?? 60_000,
      onStage: (stage) => {
        failedStage = stage
      }
