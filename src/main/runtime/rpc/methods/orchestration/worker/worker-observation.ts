...
export function resolveLocalWorkerObservation(
  ...
  const observedOptions =
    observation.exact && worker.agent_terminal_handle
      ...
      : workerIdentityNotExactObservedOptions()
  return exposeObservation(observation, observedOptions)
}
...
export async function showContextOnlyWorker(
  ...
  return {
    ...
    observation: exposeObservation(observation, {
      ...
      reason: 'no_supervised_worker' as const
    }),
...
export function exposeFederatedWorkerObservation(
  observation: {
    ...
    exactWorker: boolean
    reason?: string
    /** Rides the spread: present only when the executing host evaluated it. */
    observedOptions?: WorkerObservedOptionsObservation
  },
  projected: boolean
) {
