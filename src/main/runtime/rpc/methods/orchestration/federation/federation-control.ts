...
import {
  exposeRemoteAttachment,
  federatedObservedOptionsForAttachment,
  federatedTerminalReadStatus,
  inspectRemoteAttachment,
  requireHomeAttachment
} from './federation-attachment-observation'
import {
...
export const ORCHESTRATION_FEDERATION_CONTROL_METHODS: RpcMethod[] = [
  ...
  defineMethod({
    ...
    handler: async (params, { runtime, authenticatedCallerFingerprint }) => {
      ...
      const observation = await inspectRemoteAttachment(runtime, params.dispatchId)
      const observedOptions = federatedObservedOptionsForAttachment(
        runtime,
        attachment,
        observation
      )
  ...
  defineMethod({
    ...
    handler: async (params, { runtime, authenticatedCallerFingerprint }) => {
      ...
      const output = await readExactWorkerOutput({
        ...
        terminalStatus: federatedTerminalReadStatus(observation.status),
        terminalLiveness:
  ...
  defineMethod({
    ...
    handler: async (params, { runtime, authenticatedCallerFingerprint }) => {
      ...
      if (
        db.isUnobservedPromptAttachment(attachment, {
          requireRetainedCapability: true
        })
      ) {
