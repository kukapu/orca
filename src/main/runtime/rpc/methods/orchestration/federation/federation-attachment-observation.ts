...
/** Observed-options for a federated read: evaluated on the executing host —
 *  the one that owns the terminal and received the hook events — never on the
 *  Run home or a desktop client. */
export function federatedObservedOptionsForAttachment(
  runtime: OrcaRuntimeService,
  attachment: RemoteDispatchAttachmentRow,
  observation: Awaited<ReturnType<typeof inspectRemoteAttachment>>
): ReturnType<typeof buildWorkerObservedOptionsObservation> {
  if (!observation.exact || !attachment.terminal_handle) {
    return workerIdentityNotExactObservedOptions()
  }
  return buildWorkerObservedOptionsObservation({
...
export function exposeRemoteAttachment(attachment: RemoteDispatchAttachmentRow) {
  return {
    ...attachment,
...
/** Maps an attachment liveness verdict to the worker-output read status. */
export function federatedTerminalReadStatus(
  status: Awaited<ReturnType<typeof inspectRemoteAttachment>>['status']
