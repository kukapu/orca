...
export function getPiAgentStatusHandlerSourceLines(kind: PiAgentKind): string[] {
  ...
  return [
    ...
    '// Why (#16109): an agent restart makes the new processes inherit a dead',
    "// owner's pid, which must stay claimable or the pane reports nothing",
    '// forever. A live foreign owner (EPERM counts as alive) keeps the',
    '// suppression so inherited child agents still do not double-report.',
    'export default function (pi): void {',
    ...primeDaemonWorkerGuard,
    '  function isOwnerAlive(pid: string): boolean {',
    '    const n = Number(pid)',
